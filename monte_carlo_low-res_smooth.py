"""
The script loads radio, infrared and extinction data in lower spatial resoulution (10") 
...
def write_into_fits(inputdata, comment, outputfile):
    ...
    header['history'] = '--------------------------------------------------'
    header['history'] = 'Used script:    monte_carlo_low-res_smooth.py'
...
outpath = '../data/final_products/alpha_RIR_low-res/'
num_sim = 10000
vnu1 = 4.72e9
...
the_mask = radio_hires_lowflux_mask  # * extinction_mask

# LOAD DATA
print('Loading data')
sigma_nh = (np.sqrt(10**2 - 1.476**2) / 1.476) / 2.355
sigma_spitzer = (np.sqrt(10**2 - 2.25**2) / 1.476) / 2.355
sigma_radio = (np.sqrt(10**2 - 0.3**2) / 1.476) / 2.355

nh_map = gaussian_filter(fits.open(inpath + '3_grid_nh_repro.fits')[0].data, sigma_nh) * the_mask
sp = fits.open(inpath + '3_Spitzer_bin3_ch1_xheader_jy_subpyraf.fits')[0].data
sp[np.isnan(sp)] = 0
Spitzer_flux_map = gaussian_filter(sp, sigma_spitzer) * the_mask
Spitzer_flux_err = gaussian_filter(fits.open(inpath + '3_Spitzer_bin3_ch1_err_xheader_jy.fits')[0].data, sigma_spitzer) * the_mask
radio_map = gaussian_filter(fits.open(inpath + '3_Hires_bin3_xheader_fluxed_2009_nomask.fits')[0].data, sigma_radio) * the_mask
...
np.save(outpath + 'mc_alpha_R-IR_lowres_distr_matrix.npy', alpha_matrix)
# print('Saving Spitzer flux matrix')
# np.save(outpath + 'mc_spitzer_flux_lowres_distr_matrix.npy', spitzer_flux_matrix)
# print('Saving Radio flux matrix')
# np.save(outpath + 'mc_radio_flux_lowres_distr_matrix.npy', radio_flux_matrix)
...
outputfile = outpath + 'radio_flux_err_lowres_masked.fits'
write_into_fits(inputdata, comment, outputfile)

# SPITZER ERROR
inputdata = spitzer_flux_err
comment = 'Spitzer Flux corrected error values'
outputfile = outpath + 'spitzer_flux_err_lowres_masked.fits'
write_into_fits(inputdata, comment, outputfile)
