"""
The script reprojects 10" resolution of radio spectral index map errors 
...
hdu2_SPIXERR10 = fits.open('../data/intermediate_products/SPIXERR10.FITS')

array_SPIXERR10, footprint_SPIXERR10 = reproject_exact((hdu2_SPIXERR10[0].data[0, 0, :, :], WCS(hdu2_SPIXERR10[0].header).sub(2)),
                                               hdu1[0].header)


new_secondary_hdu = fits.PrimaryHDU()
# copy spitzer header into new secondary
new_secondary_hdu.header = hdu2_SPIXERR10[0].header
# copy chandra header projection part into new primary
hdu2_SPIXERR10[0].header = hdu1[0].header[-80:-54]
hdu2_SPIXERR10[0].data = array_SPIXERR10

# create new HDU list
new_hdul = fits.HDUList()
new_hdul.append(hdu2_SPIXERR10[0])
...
header['history'] = '--------------------------------------------------'
header['history'] = 'Used script:    3_reproject_SPIXERR10.py'
...
header['history'] = 'Reprojected file: radio/SPIXERR10.FITS'
header['history'] = 'Reproject according to file: ' + Chandra_infile
header['history'] = 'Units Jy/beam'

new_hdul.writeto(outpath + '3_SPIXERR10_bin3_xheader.fits', overwrite=True)
